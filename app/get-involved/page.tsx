...
export default function GetInvolvedPage() {
  ...
  useEffect(() => {
    fetch('/api/page-data?page=getInvolved')
      .then(res => res.json())
      .then(json => {
        if (json) setGetInvolvedData(json);
  ...
  if (!getInvolvedData) return <div className="text-center py-20">Page not available.</div>;

  const { hero, ways, programs, organizations, whyGetInvolved, stories, form, faq, finalCta } = getInvolvedData;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {ways.cards.map((card: any, i: number) => (
              <div key={i} className="card-hover bg-white rounded-3xl p-6 shadow-md border flex flex-col items-center text-center">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {programs.cards.map((card: any, i: number) => (
              <div key={i} className="card-hover bg-white rounded-3xl p-6 shadow-md border text-center">
                <span className="text-3xl block mb-3">{card.icon}</span>
                <h3 className="font-display font-bold text-lg text-deep-forest mb-2">{card.title}</h3>
