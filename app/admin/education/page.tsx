      ...
      case 'stats':
        return (
          <div>
            {data.stats.map((stat: any, i: number) => (
              <div key={i} className="flex gap-3 items-end mb-3">
                <div className="flex-1">
                  ...
                  <input type="number" value={stat.value} onChange={(e) => { const s = [...data.stats]; s[i].value = Number(e.target.value); update('stats', s); }} className="w-full border rounded-lg px-2 py-1 mt-1" />
                </div>
                <div className="flex-1">
            ...
            <button onClick={() => update('stats', [...data.stats, { value: 0, label: 'New stat' }])} className="text-sm text-emerald-green">+ Add Stat</button>
          </div>
        );
